...
def simplejson(q):
    return{
        ...
        "time": str(datetime.date.fromtimestamp(int(q['properties']['time']/1000)))
    }
...
largequakes = list(map(simplejson, largequakes))
print(largequakes)
# TODO: use the dumps() function to write json to a string
strj = json.dumps(largequakes, sort_keys=True, indent=4)
print(strj)
